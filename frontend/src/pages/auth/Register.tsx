...
export default function Register() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      login({ token: payload.token, user: { id: payload.user.id, name: payload.user.name ?? payload.user.username ?? values.name, role: payload.user.role } });
      toast.success('Registrasi berhasil');
      navigate('/dashboard');
    } catch (err: any) {
      const details = err?.response?.data?.details?.join?.(', ');
      toast.error(details ?? err?.response?.data?.message ?? 'Registrasi gagal');
    }
  };
        ...
        <div className="w-full md:w-1/2 p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="text-xs text-slate-500">
              Sudah punya akun? <Link to="/login" className="hover:text-green-600">Masuk</Link>
            </div>
          </form>
        </div>
